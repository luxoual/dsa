class Solution:
    def subarraysDivByK(self, nums: List[int], k: int) -> int:
        ...
        remainders[0] = 1  # Because 0 as a remainder is auto-pair

        for n in nums:
            total += n  # Current prefix sum
            rem = total % k  # To grab remainder

            # we can make the same amount of pairs as the number
            # of times we saw this remainder before, if its the
            # first time then we don't add anything
            res += remainders[rem]

            # Increment hte number of times we saw it
            remainders[rem] += 1
